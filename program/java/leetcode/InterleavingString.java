public class InterleavingString{
    ...
    */
    ...
    private  static boolean helper(
        ...
        if(start3 == s3.length() && start1 == s1.length() && start2 == s2.length())
            return true;
        
        if(start3 < s3.length() && start1 < s1.length() && s3.charAt(start3) == s1.charAt(start1)){
            //s3.deleteCharAt(i);
            //start3++;
            boolean rst1 = helper(s1, start1 + 1, s2, start2, s3, start3 + 1);
            //start3--;
            //s3.insert(i, s1.charAt(start1));
            if(rst1)
                return true;
        }
        if(start3 < s3.length() && start2 < s2.length() && s3.charAt(start3) == s2.charAt(start2)){
            //s3.deleteCharAt(i);
            //start3++;
            boolean rst2 = helper(s1, start1, s2, start2 + 1, s3, start3 + 1);
            //start3--;
            //s3.insert(i, s2.charAt(start2));
            if(rst2)
                return true;
        }
        return false;
